...
Git へコミットする場合、コミットメッセージは必ず日本語で記載する。

仕様変更に関するコミットでは、先頭に仕様変更IDを含め、続けて仕様変更の概要を記載する。

例:

```text
CHG-046 ドラッグハンドル並び替えを実装
```

不具合対応に関するコミットでは、先頭にバグレポートIDを含め、続けて不具合対応の概要を記載する。

例:

```text
BUG-20260326-01 起動時に画面が表示されない問題を修正
```
