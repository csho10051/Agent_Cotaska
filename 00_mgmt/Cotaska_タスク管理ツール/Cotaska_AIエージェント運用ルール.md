...
**タスクの正本は `00_mgmt/Cotaska_タスク管理ツール/data/tasks/*.md` に一本化する。**
開発タスク・アプリ運用タスクを問わず、すべてのタスクを `00_mgmt/Cotaska_タスク管理ツール/data/tasks/T-XXXX.md` で管理する。
開発タスクは `list` または `tags` で識別する（例: `list: 開発管理`）。

対象ファイルは以下の 1 系統とする。

1. タスクファイル（正本）
   - `00_mgmt/Cotaska_タスク管理ツール/data/tasks/*.md`
   - `00_mgmt/Cotaska_タスク管理ツール/data/tasks/_index.yaml`（アプリ用インデックス、AIが同期維持）
...
1. 正本を優先する
   - **全タスク（開発タスク含む）の正本は `00_mgmt/Cotaska_タスク管理ツール/data/tasks/*.md`**
...
1. **全タスクの正本**: `00_mgmt/Cotaska_タスク管理ツール/data/tasks/*.md` 群（開発タスク・アプリ運用タスクの両方を含む）
2. **インデックス（参照用）**: `00_mgmt/Cotaska_タスク管理ツール/data/tasks/_index.yaml`（アプリ用。AIは参照のみ）

開発タスクは `list: 開発管理` または `tags: ["開発管理"]` で識別するのを推奨する。

### 4.2 タスクファイル

本ルールでいう「タスクファイル」は `00_mgmt/Cotaska_タスク管理ツール/data/tasks/` 配下の Markdown ファイルを指す。

例:

1. `00_mgmt/Cotaska_タスク管理ツール/data/tasks/T-0001.md`
2. `00_mgmt/Cotaska_タスク管理ツール/data/tasks/sub/feature/T-0100.md`
...
1. `00_mgmt/Cotaska_タスク管理ツール/data/tasks/_index.yaml` を理由なく全面再生成・一括上書きする
2. タスク ID を重複生成する
3. 既存タスクファイルを無断削除する
...
1. 原則は `00_mgmt/Cotaska_タスク管理ツール/data/tasks/T-XXXX.md`
...
3. `completed_at` を設定
4. `updated_at` を更新
5. `due_date` が `null` または空欄の場合は、完了日として今日のローカル日付（`YYYY-MM-DD`）を設定する

`progress_status` を `完了` に変更する場合も、タスク完了操作と同等に扱う。
このとき `due_date` が未設定なら、CHG-054 の仕様に従って今日のローカル日付を `due_date` にセットする。
すでに `due_date` が設定されている場合は、既存の日付を上書きしない。
親タスク完了に伴って子タスクを完了へ連動更新する場合も、各タスクに同じルールを適用する。
...
4. `00_mgmt/Cotaska_タスク管理ツール/data/tasks/` に実装タスクを追加する
5. 設計書と実装を同期する
...
- id: T-XXXX
  ...
  tags: []
  due_date: null
  task_file_path: C:/WorkDevelop/Agent_Cotaska/Cotaska/00_mgmt/Cotaska_タスク管理ツール/data/tasks/T-XXXX.md
...
- `task_file_path` は**絶対パス**（Windows スラッシュ区切り）: `C:/WorkDevelop/Agent_Cotaska/Cotaska/00_mgmt/Cotaska_タスク管理ツール/data/tasks/T-XXXX.md`
- `tasks` 配列は `sort_order` 昇順で並べる
- `task_file_roots` は通常 `['.']` のまま維持する
- `progress` フィールドは廃止済み。新規作成・更新時に保存しない
...
1. `00_mgmt/Cotaska_タスク管理ツール/data/tasks/T-XXXX.md` を `list: 開発管理`（または同等のタグ）で作成
...
```text
00_mgmt/Cotaska_タスク管理ツール/data/tasks/ に新しいタスクを1件作成してください。
タイトル: ○○
期限: 2026-03-25
リスト: リストなし
タグ: ["tag1", "tag2"]
...
```text
00_mgmt/Cotaska_タスク管理ツール/data/tasks/ に開発管理タスクを1件作成してください。
タイトル: ○○
list: 開発管理
親タスク: T-0XX（必要な場合）
本文には目的と完了条件を入れてください。
...
1. `00_mgmt/Cotaska_タスク管理ツール/data/tasks/T-XXXX.md`（正本）を更新したか
...
1. ユーザーの明示指示
2. 実データの整合性維持（`00_mgmt/Cotaska_タスク管理ツール/data/tasks/*.md` と `_index.yaml` の整合）
3. `00_mgmt/AGENTS.md`
4. 本文書
