...
`progress_status` は CHG-050 により 4 値に固定する。各値の意味と使用シーンは以下の通り：
...
| **保留** | 作業対象だが現在は進めない、または一時的に止めておく状態 | 作業待ち、外部確認待ち、優先度調整待ちなど、未着・仕掛・完了と分けて管理したい場合 | `todo` |
...
2. **値の設定ルール**  
   - 上記 4 値のいずれかのみを使用
   - 「対応中」など中間状態値は追加しない
   - UI・API・frontmatter のすべての場所で同じ 4 値を使用
   - `保留` は未完了タスクの状態として扱い、`status` は原則 `todo` のままにする
...
4. **チェック↔progress_status 連動**  
   ...
   - `progress_status: 保留` はチェック操作では完了扱いにしない。保留タスクを完了する場合は、ユーザーの完了意思が明確なときのみ `status: done` / `progress_status: 完了` に揃える

5. **保留タスクの扱い**
   - `保留` は「未着・仕掛」と「完了」の間に独立して扱う進捗ステータスとする
   - 保留タスクは未完了タスクとして扱い、完了一覧には含めない
   - 左ナビの「保留」フィルターでは `progress_status: 保留` かつ `status != done` かつ `delete_flag: 0` のタスクを対象にする
   - メインペインの進捗セクションでは、保留タスクが存在する場合のみ「保留」セクションを表示する
   - 詳細ペインの進捗ステータス選択では「保留」を選択可能にする

6. **AI エージェント用の判断基準**
   - ユーザーが「完了を戻す」指示をした場合: `progress_status: 仕掛` として扱う
   - ユーザーが「保留」「一旦止める」「確認待ち」「後回し」などを明示した場合: `progress_status: 保留` として扱う
   - 4 値以外の値を見かけた場合: 未着・仕掛・保留・完了のどれに該当するかユーザーに確認してから置き換える
...
2. `progress_status` を `未着`、`仕掛`、または `保留` に戻す
3. `completed_at: null`
4. `updated_at` を更新
...
## CHG-045 parent 階層ルール

AIが `parent` を設定してタスクを追加する場合、Cotaskaの正式対応範囲は最大5階層までとする。

- 1階層目は `parent: null`
- 2階層目から5階層目までは既存タスクIDを `parent` に設定してよい
- 6階層目以上になる `parent` は原則作成しない
- 既存データに6階層目以上のタスクがある場合、Cotaskaは警告扱いにしつつ、1階層目と同じ形で表示する
- 存在しない親IDや循環参照を作らない
